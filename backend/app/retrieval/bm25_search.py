...
from app.retrieval.text_normalization import tokenize_vietnamese
...
async def _get_bm25_index() -> tuple[BM25Plus | None, list]:
    ...
    async with _index_lock:
        ...
        while True:
            ...
            if generation == _index_generation:
                break
        all_points = [
            point for point in all_points if not is_excluded_source(point.payload)
        ]
        if not all_points:
...
async def bm25_search(query: str, top_k: int = 10) -> list[dict]:
    ...
    return [
        normalize_evidence({
            ...
            "locator": p.payload.get("locator"),
            "is_active": p.payload.get("is_active", True),
