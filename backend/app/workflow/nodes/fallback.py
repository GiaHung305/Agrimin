...
async def fallback_node(state: AgentState) -> AgentState:
    ...
    if state.get("visual_observations"):
        state["final_answer"] = (
            "Tôi chưa có đủ bằng chứng truy vết để diễn giải ảnh này một cách "
            "an toàn. Tôi không kết luận bệnh chỉ từ ảnh; bạn có thể gửi thêm "
            "ảnh cận cảnh, tên cây, vị trí và thời gian xuất hiện triệu chứng."
        )
    elif state.get("retry_count", 0) >= 2:
        state["final_answer"] = (
            "Tôi chưa tìm đủ thông tin để trả lời chắc chắn câu hỏi này. "
            "Bạn nên hỏi cán bộ khuyến nông địa phương."
        )
    elif reason in _CITATION_REASONS:
        state["final_answer"] = (
            "Tôi chưa tìm được nguồn đủ phù hợp và truy vết được để trả lời "
        ...
        state["final_answer"] = (
            ...
            "tôi chưa tìm được nguồn đủ tin cậy để trả lời chính xác. Bạn nên "
